...
export default function PlanetsPaginated({ previous, next, results, slug }: PlanetsBySlugProps) {
  const router = useRouter();
  const hasNextPlanetsPage = next !== null;
  const hasPreviousPlanetsPage = previous !== null;
  const [hasMorePlanets, setHasMorePlanets] = useState<boolean>(hasNextPlanetsPage);
  const [hasPrevPlanets, setHasPrevPlanets] = useState<boolean>(hasPreviousPlanetsPage);

  const loadNextPage = () => {
    const hasNextPlanetsPage = next !== null;
    const nextPage: number = hasNextPlanetsPage ? Number(slug) + 1 : Number(slug);
  ...
  return (
    <>
      <div className="flex flex-row justify-between align-middle pb-12">
        <span className="flex justify-center h-auto w-1/4">
          {hasPrevPlanets && (
            <button
              ...
              className={
                "flex self-center py-2 px-4 border-[1px] text-sm bg-stone-900/10 border-sky-900/50 rounded-sm h-auto transition-all duration-300 ease-in-out hover:shadow-sm hover:shadow-sky-900/70"
              }
            >
              PREV PAGE
            </button>
        ...
        <h2 className="text-center text-4xl font-semibold w-2/4">STAR TREK PLANETS - PAGINATED</h2>
